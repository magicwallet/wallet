...
it('map elements correctly', () => {
  expect(
    Mapper.getSymbol(new Asset(Chain.BNB_CHAIN, ''), ProviderName.MOONPAY),
  ).toBe('bnb');
  expect(
    Mapper.getSymbol(new Asset(Chain.BSC_CHAIN, ''), ProviderName.MERCURYO),
  ).toBe('BNB');
  expect(
    Mapper.getSymbol(new Asset(Chain.BITCOIN, ''), ProviderName.MERCURYO),
  ).toBe('BTC');
});
